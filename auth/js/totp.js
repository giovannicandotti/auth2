/*
 library version jsSHA-2.3.1, sha1
...
function reset_otpgen_Form(cform){
    cform.otpcode.value ="";
    cform.secrem.value ="";
    cform.errortxt.value ="";
    cform.utctime.value ="";
    cform.localtime.value ="";
}
function clear_form(){
    reset_otpgen_Form(g_cform);
    timeout_ptr=0;
}


function showpw_Form(cform){
    if (cform.showpw.checked){
        cform.label.value= "base32 secret: "+cform.pw.value;
    }else{
        cform.label.value= "";
    }
}
...
function totp_get_otp(secret){
    ...
    return(otp); // a string of 6 digits
}
...
function update_time_fields(cform){
    ...
    g_cform.utctime.value = utcstr;
    g_cform.localtime.value = localstr;
    return(epoch);
}


// main form 
function otpgen_Form(cform) {
    cform.otpcode.value ="";
    cform.secrem.value ="";
    cform.errortxt.value ="";
    var secret=cform.pw.value;
    if (secret.length>2){
        g_secret=secret.replace(/\W/g,""); // only word char
        cform.pw.value=g_secret; // display any changes back to the user 
        g_cform=cform;
        // if there is something wrong with the secret then the sha lib might throw an error:
        try {
            cform.otpcode.value=totp_get_otp(g_secret);
        }
        catch(err) {
            cform.errortxt.value = err.message;
            return(false);
        }
        update_time_fields(cform);
    }else{
        cform.errortxt.value ="ERROR: Please enter your secret";
        return(false);
    }
    showpw_Form(cform);
    if (timeout_ptr) {
        clearTimeout(timeout_ptr); // clear existing timer if needed
    }
    timeout_ptr=setTimeout(clear_form, 40000); // clear form after 40sec, call reset_otpgen_Form with cform pointer
    return(true);
}
